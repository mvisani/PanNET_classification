...
require(ggplot2)
require(ggridges)
votes <- as.data.frame(accuracy_final_model[["votes"]])
votes["predicted"] <- as.vector(accuracy_final_model[["predicted"]])
votes["true"] <- as.vector(meta_data$CC_Epi_newLRO)
votes <- votes[votes$predicted == votes$true, ]
...
plot(accuracy_ridge, xvar="lambda", main="Ridge or L2 regression")
```
...
df <- data.frame(row.names = accuracy_important_probes)
for (i in names(coefficients)) {
  n <- as.matrix(coefficients[[i]])
  #remove first row as it is the intercept
  n <- as.data.frame(n[-1,])
  colnames(n) <- i
  df <- cbind(df, n)
  #rm(n)
