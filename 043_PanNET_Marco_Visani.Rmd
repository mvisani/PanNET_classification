...
if (!exists("meth_combat_beta"))
  meth_combat_beta <- readRDS("./data/results/20221017_meth_combat_beta.Rds") %>% as.data.frame()
if (!exists("meta_data"))
  meta_data <- read.table("./data/meta_data/training_meta_data.txt", sep = "\t", header = T)
```
...
```{r}
#TEST !!!
meta <- unique(meta_data$CC_Epi_newLRO)


set.seed(12345)
mm <- NULL
for (i in 1:length(meta)) {
  mm <- append(mm, sample(meta_data$Sample_Name[meta_data$CC_Epi_newLRO == meta[i]], size = 10))
}


sub_Set <- t(meth_combat_beta[, mm])
test_forest <- randomForest(x=t(meth_combat_beta),
...
important_probes <- as.data.frame(test_forest$importance)

require(dplyr)
important_probes <-  important_probes %>% arrange(desc(important_probes))
important_probes <- rownames(important_probes)
important_probes <- important_probes[1:10000]
...
final_forest <- randomForest(x=t(meth_combat_beta[important_probes, mm]), 
                             mtry = 100,
                             ntree = 10000)
```
