...
```{r Cross-validation for 10000 best probes}
rm(list=ls())
if(!exists("meta_data"))
  meta_data <- read.table("./data/meta_data/training_meta_data.txt", sep = "\t", header = T)
...
plot(NA, ylim=c(0,1), xlim=c(1,10000),log="x", xlab="n.probes", ylab="error.cv",
     main="Cross validation on top 10'000 probes")
for (i in 1:length(nfolds)){
  samp <- nfolds[[i]][[1]][[1]][["train"]]
  cv <- rfcv(trainx = t(meth_combat_beta[or[1:10000], samp]),
                   trainy = as.factor(meta_data$CC_Epi_newLRO[samp]), 
                   cv.fold = 3,
                   ntree =500)
...
```{r Cross validation for 2000 best probes}
rm(list=ls())
if(!exists("meta_data"))
  meta_data <- read.table("./data/meta_data/training_meta_data.txt", sep = "\t", header = T)
...
plot(NA, ylim=c(0,1), xlim=c(1,5000),log="x", xlab="n.probes", ylab="error.cv",
      main="Cross validation on top 2'000 probes")
for (i in 1:length(nfolds)){
  samp <- nfolds[[i]][[1]][[1]][["train"]]
  cv <- rfcv(trainx = t(meth_combat_beta[or[1:2000], samp]), 
       trainy = as.factor(meta_data$CC_Epi_newLRO[samp]), 
       cv.fold = 3, ntree =500)
  with(cv, lines(n.var, error.cv, type="o", lwd=2, col=i))
}
...
```{r Cross val for 1000 best probes}
rm(list=ls())
require(ggplot2)
if(!exists("meta_data"))
  meta_data <- read.table("./data/meta_data/training_meta_data.txt", sep = "\t", header = T)
...
plot(NA, ylim=c(0,1), xlim=c(1,1000),log="x", xlab="n.probes", ylab="error.cv",
     main="Cross validation on top 1'000 probes")
for (i in 1:length(nfolds)){
  samp <- nfolds[[i]][[1]][[1]][["train"]]
  cv <- rfcv(trainx = t(meth_combat_beta[or[1:1000], samp]), 
       trainy = as.factor(meta_data$CC_Epi_newLRO[samp]), 
       cv.fold = 3, ntree =500)
  with(cv, lines(n.var, error.cv, type="o", lwd=2, col=i))
}
...
cv <- rfcv(trainx = t(meth_combat_beta[or[1:1000], ]), 
       trainy = as.factor(meta_data$CC_Epi_newLRO), 
       cv.fold = 3,
       ntree =500)
with(cv, lines(n.var, error.cv, type="o", lwd=2, col=i+1))
