...
* 2'000 probes, 1'000 trees (not shown)
* 1'000 probes 1'000 trees
* 1'000 probes 1'000 trees but joining Intermediate_ADM_WT and Intermediate_WT into one class
...
confusion_matrix <- function(data=data){
  require(pheatmap)
  require(RColorBrewer)
...
probes_10 <- CV_result(data=probes_10)
test_roc <- ROC_graph(probes_10)
test_confusion <- confusion_matrix(probes_10)
```
...
load(file.path("out_10_probes","CVresults.RData"))
...
Should be read from right to left: there are 6 Intermidiate_ADM that were classified in the Alpha_like class. 
...
# 1'000 probes Classifier cross-validation 
Batch effect was NOT taken into account here !
...
* Is the consensus clustering and good response ? 
* Should the wrongly classified samples be looked again clinically ? reanalyzed ? 
* ... 
...
require(ggplot2)
require(ggridges)
calibrated_score <- as.data.frame(probes_10$probs)
calibrated_score <- calibrated_score[probes_10$y.true == probes_10$y.calibrated, ]
max <- apply(calibrated_score, 1, max)
Four_classes <- colnames(calibrated_score)[apply(calibrated_score, 1, which.max)]
calibrated_score <- cbind(calibrated_score, max)
calibrated_score <- cbind(calibrated_score, Four_classes)
