...
plotCCheatmap(cons_cluster, 
              ...
              colorList = list(Grade = c(G1 = "khaki", G2 = "blue2"),
                               DAXX_ATRX = c(wt = "white", mut = "goldenrod4"),
...
## Confusion matrix
After training of the classifier with the top 10'000 probes, the random forest algorithm classified the samples as follows. 
Questions: 
\item Is the meta data actually the grand truth ? 
\item Should the wrongly classified samples be looked again ? 
\item ... 
...
m <- rf.pred$confusion[, -ncol(rf.pred$confusion)]
m <- m/apply(m, 1, sum)

pheatmap(m,
         display_numbers = rf.pred$confusion[, -ncol(rf.pred$confusion)],
