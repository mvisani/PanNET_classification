...
The random forest was developped by 
...
par(mfrow=c(3, 2))
for(i in unique(y)){
  plot(x=scores[ys==i,i],
       y=probs[ys==i,i],
       ylim=c(0,1),pch=16,xlim=c(0,1),
       col=as.factor((yp==ys)),
       main=i,
       xlab="scores",ylab="calibrated scores")
  abline(a=0,b=1,col="green",lty=2)
}
```
...
After having followed the protocol of Capper et al. (https://doi.org/10.1038/nature26000), the question arose of knowing if 10'000 probes were to many and could risk an over fitting. A cross validation on 2'000 and 1'000 probes was tested:
