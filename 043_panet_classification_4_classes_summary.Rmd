...
# TL;DR 
First read `043_panet_classification_5_classes_summary.Rmd` for detailed explanation of the results and the project. 
We tested the same pipeline but merging 2 classes into one (Intermediate_ADM_WT and Intermediate_WT). The results are more or less similar.  

Consensus clustering was run with $k=4$ clusters. 
...
For further details or questions, please see my [GitHub](https://github.com/mvisani/PanNET_classification) page or send me an email at [marco.visani@unifr.ch](mailto:marco.visani@unifr.ch).
...
![image](./results/image/4_classes/tree_selection.png)
...
![image](./results/image/4_classes/probe_selection.png)

Same as for the 5 classes, above 100 probes, the accuracy remains the same. 
...
Here once again, we can clearly see that below 50 probes, the classifier is not accurate. Most Alpha like are classified as Intermediate_ADM. Otherwise, accuracy seems to be a bit better than for the 5 classes. 
...
Once again, score calibration doesn't help our model. For now and for predicting new data (see below), the raw score are kept. In the future, when more data is added, score calibration will have to be performed. 

## Consensus clustering
```{r}
require(pheatmap)
cons_cluster <- readRDS(file = "./data/raw_data/220609_NDD.consClust_DMP.Rds")
cons_cluster <- cons_cluster[[4]]
```
...
There is less disagreement between the random forest and the consensus clustering. This is probably due to the fact that the random forest has a bigger sample group per class to train with. Another reason could be that the it is easier to classify the samples since there are only four classes instead of five.
...
Interestingly, the DMPs chosen by the classifier are completely different than the ones in the 5 classes case. 
...
Here accuracy is much higher than for the 5 classes. The reason could be that the joining of two classes improved a lot the accuracy of the random forest. It could also be possible that the fact of joining two classes increases the chances of assigning a sample at random and being correct. 
...
As accuracy showed, there is less disagreement in the 4 classes than in the 5 classes case. 
