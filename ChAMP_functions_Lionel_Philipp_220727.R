                                ...
                                dataToInclude = c("loadQC", "mset", "mset_unfilt", "rgSet", "pd", "intensity", "beta", "detP")) 
{
  ...
  if (method == "minfi") {
    if (is.null(rgSet_object)){
      ...
      stopifnot(class(rgSet) == "RGChannelSetExtended")
    }
    ...
    if (preproc == "Raw"){
      ...
      mset = preprocessFunnorm(rgSet, keepCN = F, ratioConvert = F) # the return type is a genomic methyl set 
    } else if (preproc == "ENmix"){
      message("EMmix background, dye bias and probe type correction")
      mset = preprocessENmix(rgSet) # according to the documentation the return type is again an RGset
      mset = preprocessRaw(mset) 
    }
    ...
    myLoad <- list(mset = mset, mset_unfilt = mset_unfilt, rgSet = rgSet, pd = pd, intensity = intensity, 
                   beta = beta.raw, detP = detP, loadQC = loadQC)
