...
if(!file.exists(file.path("..", "CV_1000","nfolds.RData"))){
  dir.create("../CV_1000",showWarnings = FALSE)
  nfolds <- makenestedfolds(y,folds)
  save(nfolds,file=file.path("..","CV_1000","nfolds.RData"))
}
...
for(K in 1:folds){
  ...
  for(k in 0:folds){
    ...
    save(rf.scores,file=file.path("..", "CV_1000",fname))
