...
rfp <- function(xx, ..., ntree = ntree, mc = mc, seed = NULL)
{
  ...
  rfwrap <- function(ntree, xx, ...) randomForest::randomForest(x=xx,ntree=ntree,norm.votes=FALSE, do.trace = 500, ...)
  rfpar <- mclapply(rep(ceiling(ntree/mc),mc),mc.cores=mc, rfwrap, xx=xx, ...)
  #rfpar <- mclapply(rep(100, ntree/100),mc.cores=mc, rfwrap, xx=xx, ...)
