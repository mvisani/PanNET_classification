...
load(file.path("..","CV_1000","nfolds.RData"))
...
for(i in 1:length(nfolds)){
  ...
  save(probs,file=file.path("..","CV_1000",fname_probs))
}
...
save(cv.calfit,file=file.path("..", "results","calfit_1000.RData"))

save(scores,probs,y,ys,yp,file=file.path("..", "results","CVresults_1000.RData"))

message("generating report ...",Sys.time())
rmarkdown::render("CVresults_1000.Rmd")
